...
def handler(event, context):
    ...
    if (event['detail-type']=="EMR Cluster State Change" and event['detail']['state'] in state):
        ...
        if ids:
            ...
            for response in paginator.paginate(Dimensions=[{'Name': 'InstanceId', 'Value': master_instance}],MetricName='StatusCheckFailed',Namespace='AWS/EC2'):
                if response['Metrics']:
            ...
            for id in ids:
                for response in paginator.paginate(Dimensions=[{'Name': 'InstanceId', 'Value': id}],MetricName='CPUUtilization',Namespace='AWS/EC2'):
                    if response['Metrics']:
                        for d in response['Metrics']:
                ...
                for response in paginator.paginate(Dimensions=[{'Name': 'InstanceId', 'Value': id}],MetricName='DiskSpaceUtilization',Namespace='System/Linux'):
                    if response['Metrics']:
                        for d in response['Metrics']:
                ...
                for response in paginator.paginate(Dimensions=[{'Name': 'InstanceId', 'Value': id}],MetricName='MemoryUtilization',Namespace='System/Linux'):
                    if response['Metrics']:
            ...
            # else:
            obj = s3.Object(os.environ['BUCKET'], 'emr/bootstrap/lambda-code/dashboard.json')

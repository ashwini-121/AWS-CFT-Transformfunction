    ...
    if (event['detail-type']=="EMR Cluster State Change" and event['detail']['state'] in state):
        ...
        if ids:
            ...
            for id in ids:
                ...
                for response in metrics_paginator.paginate(Dimensions=[{'Name': 'InstanceId', 'Value': id}],MetricName='MemoryUtilization',Namespace='System/Linux'):
                    if response['Metrics']:
